...
| U-019 | Official template required/optional fields, types, enums, bounds, and additional-property rules | Artifact validation | Formal schemas or dated authoritative clarification. **Still open, but 2026-08-05 the coordinator authorised building the `M7-002` artifact builders against the documented (unauthenticated) template in `JSON_ARTIFACT_SCHEMAS.md`, marked `U-019`-PROVISIONAL** — the exact field set may change when this is ruled; the builders are not treated as the final schema |
...
| U-025 | ~~Emission value of the eight `5×5` cells at squared-distance 5 (the `(±1,±2)`/`(±2,±1)` ring)~~ | ~~Scent emission (`M6-001`), and any belief update that reads the raw field~~ | **CLOSED 2026-08-05 — resolved by negotiation, not by a ruling.** Re-verified directly against the book summary (`inst/police_thief_p2p_Summary.md:947-955`): Figure 4 names five radial classes covering **17 of 25** cells and gives these eight **no value**. Nothing can derive one, so this was never waiting on a ruling. The book prescribes the alternative itself (PDF p. 31, `…Summary.md:1043-1048`): **agree** the emission and decay model, confirm both sides interpret it identically, lock it with SHA-256 — and it *recommends* handing the opponent your scent source code. The ring is therefore a negotiated parameter carried inside the `M6-005` lock, replacing `_PROVISIONAL_D2_5`. **Note the ID collision:** the companion Cop repository registered this same question as `U-030`, while its own `U-025` is the (closed) six-sub-game role schedule — the two registers are not a shared namespace and never were. | **A NotebookLM answer claimed Figure 4 prints all 25 cells, with diagonals `0.42` and this ring `0.14`. The book summary contradicts it on every point and the answer was rejected** — implementing it would have replaced a correct emission table with an invented one. **REOPENED AND RE-CLOSED 2026-08-08 — the premise was false.** Figure 4 names **all 25 cells**, and this ring is `0.14`. The reason it looked unnamed is that the rest of our table was the correct curve **shifted inward by one radial class**: our diagonal `0.20` and mid-side `0.14` are the true values of the next two rings out, so the shift consumed the class that owns `0.14` and left eight cells with nowhere to come from. Confirmed three ways: the book **PDF** states the six classes directly; a zero-parameter fit of `τ = 0.9·exp(−k·d²)` through the only two values every reading agreed on (centre `0.90`, cross `0.62`) yields `0.427 / 0.203 / 0.140 / 0.046`, matching to two decimals; and a classmate team reproduced the same kernel independently. **The 2026-08-05 closure rejected these exact values on the grounds that "a notebook answer is not a source" — but the notebook holds the PDF and the file it was checked against is a translation.** The rule was right and was applied backwards; a restatement is not the source in either direction. The kernel, the lock hash (`416a57e1…` → `e6aef097…`), the stored vectors and every measured result were regenerated. `test_scent.py` now pins the *curve* as well as the table, so a future shift fails without needing a source to argue with. |
...
| U-026 | Whether the replay audit must **detect** a structurally damaged log — records reordered, one deleted, one duplicated — when every individual digest still verifies | `M8-008d`; the replay banner's meaning, and which sanction applies | **Asked directly 2026-08-06; both sources are silent by design, so this is a recorded project decision.** Book: rule 19 is "any mismatch **in the digest**" (p.129/271); no rule requires contiguous numbering; a missing step is instead contradictory reports (rule 35, p.131/275) plus an illegal state jump (rule 5). Reference: each record is verified independently "with no reference to its place in the sequence", `normalize_log` does not sort or re-index, and no code rejects a duplicate or missing step. **Decision: detect and report, never banner** — `replay/sequence.py` emits rule-tagged findings while the `Verified OK` / `TAMPERED` stamp stays digest-only, because the sanctions differ (rule 19 punishes the falsifying group, rule 35 punishes **both**) and red-bannering an opponent over ordering would be a false accusation with no appeal (`:1769`). Reopen if the lecturer states that ordering is itself auditable. |
| U-024 | Whether an opponent may be **refused** for omitting the book's mandated pre-game identity content | `M5-014f`; every match against a peer that keeps identity in artifacts rather than on the wire | **OPEN, and this peer currently answers "yes" where the companion answers "no".** The book mandates the content (`AE-24`, sanction: a technical loss for an incomplete pre-game exchange), so *populating our own* offer is not in question. What is unresolved is the inbound half. `protocol/handshake.require_identity` refuses a peer whose block omits any of the seven mandated members; the companion Cop **tolerates** the same omission and plays on, recording the split as its `C-031`. Verified by running both on 2026-08-07. The risk is asymmetric and real: a simulator-built classmate that emits identity into artifacts instead of onto the wire plays a full series against our Cop and is refused by our Thief at the handshake, which reads to them as an unresponsive peer. Neither behaviour is wrong on the text — the rule sanctions *sending* an incomplete exchange and says nothing about receiving one — so this needs a coordinator ruling rather than a unilateral change. Recorded here on 2026-08-07 after `check_ledger_consistency.py` found `M5-014f` citing a number this register never carried |
| U-033 | Whether the cumulative **draw award** was meant to reach a series decided entirely by **technical losses** | Series scoring; the emailed result artifact | **OPEN.** Registered here 2026-08-07 because `M9-020b` claims to list every unresolved unknown and cited this number, which this register did not carry — the citation resolved in the companion repository and nowhere here, so a reader of *this* repository would have found a dangling reference. The question is the companion's finding and is recorded at that provenance; this peer has taken no position on it and its scoring is unaffected, since a technical loss is scored 0/0 by table 2 and rule 48 regardless of how the draw award is read |
| U-034 | Whether an **empty `smell_grid`** should be refused on the wire | Inbound turn validation; the shared turn schema | **OPEN.** Same provenance as `U-033` and registered for the same reason. `{}` currently validates, because the schema types `smell_grid` as an object without requiring members. Silence and an empty field are indistinguishable on the wire, and this peer's parser treats both as "no scent observed" rather than as a protocol error — which is the conservative reading, since refusing would make a peer that legitimately emits nothing look like a forger |
| U-031 | Whether re-emission on an **occupied** cell **accumulates** per the printed formula or is **clamped** to the `0.9` centre intensity | Scent update semantics in `perception/scent.py`; any cross-peer field recomputation | **OPEN.** Registered here 2026-08-15, taking the companion's number because `U-033`/`U-034` already share theirs. The book prints `max(0, (1-rho)*tau + delta)` with a lower clamp only, and separately declares tau continuous in `[0, 0.9]`; the two readings differ the moment an agent re-emits on a cell it already scented. This repository implements the printed formula with **no** upper clamp. Surfaced by the `imreeyal`/`anrbj666` conformance kit, whose `ref_book_update` clamps -- so two implementations both faithful to the book produce different fields. Not audit-visible (the grid rides unauthenticated) and not belief-visible here (the `M11-001` decoder reads window shape, not values), but it becomes load-bearing under any opt-in grid binding. The gap was that this register had no row while the companion did |
