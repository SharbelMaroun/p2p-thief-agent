...
| U-023 | Whether the shared match object must carry `pheromone_min_center_intensity`, and at what value | Pre-play agreement; cross-peer interoperability | **OPEN — coordinator decision. Raised 2026-07-31.** This repository already treats `min_center_intensity` as a **required** agreed term (`REQUIRED_TERMS` in `protocol/handshake.py`), which the pinned wire reference confirms is right: its `terms_from_config` always writes the key, assigning `None` when the config lacks it, the key is in its own required list, and a `None` value aborts at startup with `Missing required agreed game term(s): ...`. Its shipped `config/police/game.json` and `config/thief/game.json` both carry `pheromone_min_center_intensity`; the exact numeric value was not established. Appendix F table 16 fixes centre intensity `0.9`, decay `0.10`, and the 5x5 field but defines **no** minimum-centre floor, so the book alone does not mandate the term — which is why the companion peer currently treats it as *optional*. **The two repositories therefore disagree**, and any shared match object we hand a classmate must settle it. Whoever holds the shared bundle decides; this agent recorded the conflict rather than picking a side |
