...
| M6 | Scent, belief and private strategy | Confirmed scent physics, public observations, Thief-local belief, and private strategy | `DONE` |
...
| M8 | GUI, replay, interoperability and security hardening | Local-truth UI, replay/verifier, neutral-opponent E2E, tamper tests, and security review | `DONE` — closed 2026-08-12 when `M8-003c` (rehearse against a real classmate) landed its evidence: six live Thief-role games against `uoh-ay26` across two complete six-game series, every audit accepted, the second series with zero wire rejections. All 59 M8 rows are `DONE`/`SUPERSEDED`; the belief-map/replay screenshots remain tracked under `M5-005c` |
