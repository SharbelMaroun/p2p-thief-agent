...
| M5-019f | Sequence negotiation and first move autonomously | PENDING | **Not done, and deliberately not stubbed.** Hosting and readiness are now done (`M5-019e`); what remains is the *protocol* half. Confirmed 2026-08-02: the reference's runtime **waits for the opponent's counter-signature before step 1** — "play starts only after both verifications pass" — so this needs send-offer → poll the agreements inbox → verify both directions → begin play. The book adds two requirements the reference does not settle: the Step-0 attestation must be **exchanged and mutually signed**, and the pre-game declaration (`_schema`, `schema_version`, `declaration_type`, `game_id`, `game_uid`, `links`, `timezone`, `game_started_at`, `game_ended_at`, `num_sub_games`, `max_tokens_per_game`, `groups`) must be written **after negotiation but before play**. **Thief-specific:** this peer *opens*, so once negotiation completes it must send step 1 without waiting — a Thief that waits deadlocks against a Cop correctly waiting for it. A **passive** `serve` was rejected in the companion repo on 2026-08-01 as proving connectivity rather than a game; that decision is honoured here |
