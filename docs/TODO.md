...
**2026-08-09 first live opponent — for the Cop, not for this agent.** A friendly,
uncounted series against group `amireman` began that day, but sub-game 1 was played in
the **Police** role. Only `M9-005` closes here, on its own evidence: the annotated
`v1.0-submission` tag is pushed. `M8-003c` and `M5-005c` stay `PENDING` **by
`THIEF-002`** — the game that would close them was played by the companion repository,
and this ledger does not accept its evidence. They close on the first sub-game played in
the Thief role. **M9 has not started**: the counted-league, identity, dry-run, and
...
**2026-08-12h the flawless series, and the rows it closes.** The second complete six-game
series (`games/friendly-uohay26-0812-2201/`) ran with **zero wire rejections end to end** —
first ever — reproducing the first series game for game: survival at 35 in every Thief
game, the companion capturing at 15/17/16. Their `series_consensus` finale was accepted
(companion `C-040`'s first live pass), their converter survived every companion Police
audit (`C-042`), and seven audits crossed with nothing refused in either direction.
`M8-003c` closes on this evidence; `M5-005c` narrows to its two M8 screenshots. Rule 32's
pipeline was also proven live: the companion's result artifact for the first series was
...
| G-010 | Keep `DOCS_COMPLETENESS.md` reconciled after each milestone | DONE | `scripts/check_ledger_consistency.py` fails if a document exists without a row or a row claims a missing file. It found **29 missing rows**: the table was last reviewed 2026-07-28 when this was still an M1 scaffold, and every row still in it was individually accurate, which is why the gap survived. Rewritten to 52 rows including each ADR with its own status; gate-scoped verification records excluded, since a "current maturity" for `M1_VERIFICATION.md` would need re-dating forever |
| G-011 | Keep `PLAN.md` milestone states consistent with this ledger | DONE | Same script, checked in **both** directions — `DONE` in the plan with open tasks overstates progress, and an open state with everything closed understates it, which is worse for a grader who reads the plan and stops looking. Found `PLAN.md` calling M3 `DONE` with `M3-005c` still open. The parsers are this repository's own: the companion puts the milestone state in column two and this one in column four, so shared code would be wrong for one of them |
| G-012 | Keep `REQUIREMENTS_LEDGER.md` in step with implemented behaviour | DONE | Same script: every `test_*.py` cited in `REQUIREMENTS_LEDGER.md` must exist. Clean — it bites only when a row names a file, which is exactly when a rename would strand it |
| G-013 | Keep `UNKNOWN_REQUIREMENTS.md` in step with blocking tasks | DONE | Same script, both directions — and it found three real breaks. `TODO.md` cited `U-024` (`M5-014f`), `U-033` and `U-034` (`M9-020b`), **none of which this register carried**; they resolved in the companion repository and nowhere here, so a reader of this repository found a dangling reference where `M9-020b` claims to list every unresolved unknown. All three now registered. `U-024` records a genuine cross-peer divergence found while writing it: this peer **refuses** an opponent whose pre-game identity omits mandated members, the companion **tolerates** it, so a classmate that keeps identity in artifacts plays our Cop and is refused by our Thief. Verified by running both 2026-08-07; left open for a ruling rather than changed unilaterally, since rule 24 sanctions *sending* an incomplete exchange and says nothing about receiving one |
| G-014 | Keep ADRs current when a decision changes | DONE | Same script: every ADR carries a `Status` line, and one discussing supersession records it there rather than being quietly rewritten. All nine are statused; `Pending` and `Proposed` are deliberate — an unaccepted decision is recorded as unaccepted |
...
| M0-006 | Record the book's internal contradictions the report must disclose | DONE | `docs/SPECIFICATION_CONFLICTS.md` holds the register (`C-001`…`C-020`) and `docs/HANDOVER.md` discloses the six the code actually relies on, under chapter 110's clause. **Closed late**: the register has existed since M6, and `M9-011c` was briefly closed against a subset of it — recorded rather than backdated |
| M0-006a | Record the appendix-lettering inconsistency | DONE | `C-021` in the register: the parameters table is called Appendix F in the body and lettered differently in the contents. Only the table binds; the letter does not change which values are Fixed |
| M0-006b | Record the board-size illustration inconsistency | DONE | `C-020`: the binding board size is 7×7 (Appendix F table 13) while illustrations use 10×10, 6×6, 5×5 and 3×3. Build at the negotiated size; treat every illustration as illustrative |
| M0-006c | Record the `[Number of Agents]` name collision | DONE | `C-018`: `[Number of Agents]` names two different parameters — table 13 row 2 is players in a match (2, Fixed), table 18 row 1 is games in a series against one opponent (6, Fixed). Reading the wrong row gives a series of 2. Disclosed in `HANDOVER.md` as contradiction 2 |
| M0-006d | Record the series-of-six versus one-scoring-game tension | DONE | `C-017` and the series/scoring tension: table 18 fixes six sub-games while the template ships `num_games: 1`. We play six and say why |
| M0-006e | Record the missing technical-loss row in Table 17 | DONE | `C-019`: Appendix F table 17 lists five scoring rows and omits the technical-loss value the config schema requires. Fixed at 0/0 by table 2 (p.38) and rule 48 |
...
| M1-025 | Keep the JSON artifact schemas documented | DONE | `tests/unit/test_artifact_schema_doc.py` (7 tests) checks every field the builders require against the section of `JSON_ARTIFACT_SCHEMAS.md` describing that artifact. It found the document already wrong twice: `os` (rule 24, `inst/:1278`) and declaration-level `github_commit` (rule 53) were added to the builders on 2026-08-07 and never to the document, which still read as current. The check is **per section**, because a document-wide search finds `github_commit` under `4-final-result.json` and reports the declaration as documented — shared field names are precisely what a loose check waves through. Direction is emitted ⊆ documented, so the document may still discuss template fields we deliberately do not emit; `U-019` stays open on provenance |
...
| M3-005c | Disclose the choice in the academic report | DONE | `docs/ACADEMIC_REPORT.md` §1.2, third disclosure block. **Where**: Appendix F table 15 sets `[Step Limit]` and `[Survival Threshold]` to the same value, 35 — in the mandatory parameters table both peers negotiate from, not in a figure. **What**: the inclusive horizon, completing step 35 uncaptured is a win. **Why**: chapter 3 table 2 defines survival as lasting "the limit of valid moves", and table 15 makes that limit equal the threshold. For this agent it is not a corner case but the **entire win condition**. The companion Cop records the identical reading as `C-024` with its own boundary test, so both sides of a match score the final turn the same way `[book p.5]` |
...
| M5-005 | Validate localhost and public-tunnel paths against identical fixtures | DONE | `adapters/serve.resolve_peer` + `p2p-thief serve --private game.toml`. Book 2.4 (p.13): "running servers on localhost is permitted only during the early development stages. In practice, each group must expose its FastMCP server to the public internet using tunneling tools, such as ngrok or Localtonet." Both paths now read the same way -- a flag for local development, the private TOML for league play. An explicit `--peer` wins over the file, because an operator typing an address means that address |
| M5-005a | Keep tunnel credentials out of shared configuration | DONE | `ADDRESS_MEMBERS` already guarded the names; `test_public_url.py` now asserts it covers `public_url`, `tunnel_url`, `bind_host` and `port`. Only the resulting URL is exchanged -- the provider and its authtoken stay in the private TOML, because rule 39 forbids pushing secrets even to a private repository `[AE-39]` |
| M5-005b | Exchange only the public URL | DONE | `shared/tunnel.public_url`, split from `private_config` because it answers a different question: not *what may live in the private file* but *what we advertise and what must never travel with it*. **Refuses a loopback address** -- `127.0.0.1`, `localhost`, `0.0.0.0`, `::1`, `127.x`. That is the failure worth moving early: a loopback URL is valid, negotiable and signable, and fails only at the first turn, by which point both sides have committed to it. The Cop calls binding `127.0.0.1` "the one-word bug no local test would ever catch" `[AE-10]` |
| M5-005c | Rehearse a full game across two machines | PENDING | Book stage-5 milestone: "Agent on a remote computer connects via ngrok and plays a full game against the local agent" (`police_thief_p2p_Summary.md:2458`). Three blockers, one now cleared: the *play loop* is **RESOLVED** by `M5-019`; *launching a peer* is `M5-019e`; *hardware* (two machines, a live tunnel) cannot be unit-tested. **Found 2026-08-02 and confirmed at `police_thief_p2p_Summary.md:2295`:** the mandated **evidence** is "Live GUI (belief map) and Replay App (Verified OK) screenshots", both **M8** deliverables — so this cannot be *evidenced* until M8 exists even with the hardware and CLI in place. **2026-08-09: the hardware blocker stopped being hypothetical.** The companion repository played a full game across two machines against group `amireman` over a Cloudflare tunnel, and its `replay` printed "Verified OK" over 34 steps — so the tunnel path, the peer launch, and the *Replay App* half of the mandated evidence are all demonstrated to work. What is still missing **here** is the same run in the **Thief** role plus the two screenshots. **2026-08-12 evening: the Thief-role run exists** — six full games over live tunnels in two complete series (`0812-1934`, `0812-2201`), logs replay `Verified OK`. All that remains of this row is the two screenshots, an M8 capture task |
...
| M5-010 | Handle opponent-side rejection responses | DONE, **wired 2026-08-09** | `PeerRejectionError` vs `TransportError` are disjoint (`adapters.fastmcp_client.signals_refusal`); `orchestration/delivery.deliver` retries only transport faults; a rejection terminates the sub-game (`test_delivery.py`, `test_sub_game.py`). **This row was true of the helper and false of the match until 2026-08-09**: `deliver` had **no production call site**, so both live sends made one bare attempt — a single tunnel 502 mid-match ended the sub-game 0/0, and one on the pre-game offer sent a raw `TransportError` out through `serve_match` (the shape `PROMPT_LOG.md` records for the first real match attempt). Now bound via `delivery.retrying_deliver` at both sites — `adapters/serve.serve_match` (turn send) and `adapters/negotiated.negotiated_agreement` (offer send). `test_transport_retry_wiring.py` pins both against regressing to one attempt |
| M5-010a | Distinguish rejection from transport failure | DONE | `deliver` uses `retry_on=(TransportError,)`: a transient transport fault is retried to the agreed limit, a `PeerRejectionError` propagates on the first occurrence; `test_delivery.py`, and `test_a_rejection_is_never_retried` proves the distinction survives the live wiring |
...
| M5-014f | Enforce the book's mandated Step-0/negotiation content and the `config_sha256` lock `U-024` | DONE | `protocol/handshake.require_identity`, wired into `verify_peer`; `tests/unit/test_identity_content.py` (13 tests). **The gap was inbound.** `identity_block` already takes all seven mandated members as required arguments, so our own could not be short — but an opponent's arrived as `message.get("identity", {})` and an empty dict was accepted in silence. The first sign would have been a declaration we could not complete, *after* both sides had signed the terms. Now refused at the wire, naming every missing member at once so a handshake stays one round trip. `inst/:1278` and p.39/104 mandate the content; rule 24 is Mandatory and charges a short identity with denial of the computational bonus. Adding it broke 4 fixtures that had been agreeing with no identity at all `[AE-24]`. **`U-024` stays open**: whether `config_sha256` must be an explicit wire member is a contract question, and deciding it unilaterally would bind an opponent to a shape no source requires. The terms are already locked through the signature |
...
| M5-020 | Negotiate on the playable path, and wait like the loop's contract says | DONE | **Closed 2026-08-08, by the first two-process rehearsal of the real policies.** `M5-019f`'s sequencing existed and only the tests ever called it — `serve --peer` went straight to the turn loop, which composes with nothing: the companion Cop (and the book) refuse an unnegotiated game. Now `serve --game SHARED.json --private game.toml` projects the shared file into the reference's flat signed terms (`protocol/terms_projection.py` — `game_id`/`game_uid` deliberately excluded, they would break every cross-peer signature), builds the rule-24 identity from the private TOML (`identity_from_private` — refuses a short one by member name), runs the handshake over the live pair (`negotiate_for_serve`), and plays to the **negotiated** horizon. Second defect the same rehearsal exposed: `serve_match` passed the raw non-blocking inbox take as the loop's `receive`, whose contract is that `None` means the deadline truly passed — so the loop checked the inbox once, microseconds after its own send, and declared a live opponent silent (technical loss at step 1, both sides). `receive` is now `poll_for_turn` bounded by the shared file's `response_timeout_sec`. `tests/unit/test_negotiated_serve.py` (8: the exact projection roster, refusal-by-name both ways, bounded silence, the in-memory handshake round trip) |
...
| M6-002 | Consume accepted public scent observations in the accepted order | DONE | `perception/observation.py` (100% branch); `test_observation.py` covers shape, order-independence, the sparse "absent, not zero" rule, deterministic encode order, round-trip, and boundary handling. Off-board handling is `M6-006b` |
...
| M6-005 | Lock and exchange the scent-model hash before the first move | DONE | `perception/scent_lock.py`: the full model (formula, FIXED constants, field size, and the complete 25-cell profile including the negotiated `U-025` ring) is canonicalised and SHA-256 locked. **Reworked 2026-08-05** — the lock left the signed terms and the ring became a parameter, so the record now describes an *agreed* model rather than this peer's private constants. The digest `e6aef097…` is pinned in `test_scent_lock.py` and is reproduced exactly by the independently written Cop peer, which is the only real evidence that a lock is worth anything `[AE-23]` |
...
| M6-006b | Parse an opponent field defensively | DONE, corrected 2026-08-09 | `parse_smell_grid(grid, board)` rejects a malformed key and a non-numeric/negative intensity — genuine corruption. A cell off the negotiated board is **dropped, not rejected**: the row previously read "rejects... any cell off the negotiated board", which whole-grid-rejects a peer's legitimate fixed-size window near any edge or corner — reproduced 2026-08-09 as the confirmed root cause of the companion Cop repository's lost friendly match (`orchestration/thief_absorb.py` had the matching swallow-to-silence pattern; this repo's own `cop_start` is the corner `[0,0]`, so the same defect shape would have blinded this Thief from turn 1). `parse_smell_grid_dropped_count` keeps the discard rate visible so a wholly-wrong grid stays observable. `test_observation.py`, `test_thief_policy_off_board_scent.py` |
...
| M6-015c | Re-state the evasion acceptance criterion in league points, not survival steps | DONE | **The criterion was restated and the policy then failed it, so the policy was fixed.** `M6-015` accepted belief-driven evasion on *total survival steps* over four hand-picked openings; over all 24, in the league points Appendix F actually pays, it scored **140 against the blind baseline's 175** — worse than a random walk at the only thing counted. Root cause: `choose_evasive_action` delegated to a **lexicographic** ranking with threat distance first, so mobility could only separate equally-distant moves. Maximising distance on a bounded board against a pursuer walks into a corner — distance large, exits zero. It now **sums** distance and mobility (weights 1 and 1, nothing fitted): **235 points, 23/24 escapes, paired 13-0-11**, and `metric_disagreement` is now `false`. `mobility` not `edge_contacts` because the Cop places up to 14 barriers and only mobility counts them. Re-checked on board sizes 5–9, randomised openings rather than the tuned set, barrier layouts, and horizons 15–50 — the advantage holds and **grows with the horizon**, because the old ceiling near 28 steps is invisible whenever the threshold sits below it. Acceptance test now asserts league points over the full opening set; academic report §3.1 and §2.2 corrected, `RESEARCH-REPORT` table corrected with the previous numbers shown `[AF-002]` `[AF-004]` |
...
| M7-005f | Run the full mutual audit before agreeing a result | DONE | `email_report._require_agreed` — `compose_report` now **takes the settlement record** and refuses any state short of `agreed` with `audit_passed is True`; `test_report_precondition.py` (15 tests). The gap was a layering one: `settlement.agree` already took its audit first, but `compose_report` took a bare result mapping, so the ordering lived in the caller. `is not True`, not truthiness — a JSON round-trip turning the flag into `"True"` would otherwise pass `[AE-35]` `[AE-36]` |
...
| M7-008 | Attach every game's configuration artifact to the repository | DONE | `reporting/retention.py` (100% branch); `test_retention.py`, `games/README.md`. **We were violating Appendix F obligation 4**: `.gitignore` excludes `logs/`, `reports/generated/` and `results/generated/`, so a config written under any of them was retained on one laptop and lost to the repository — silently, since the write succeeds |
| M7-008a | Commit each game's config under a `game_id`-derived name | DONE | `store_config` writes to `games/<game_id>/config_<game_id>_g<NN>.json` and **refuses a destination under an ignored path**. Matched as a run of path components so `build/logs/games` is caught; `results/generated/` is ignored while `results/` is not, and refusing the parent would be a false alarm |
| M7-008b | Prove any past game's config is retrievable from the repo | DONE | `retrieve_config` round-trips any past sub-game; `missing_configs` returns **every** gap rather than raising at the first, because that is the pre-submission question. `test_retention_retrieval.py` |
| M7-009 | Account for LLM tokens across a series | DONE | `reporting/token_ledger.py` (100% branch); `test_token_ledger.py`, `test_token_limits.py` (32 tests). Rule 54 wants both figures and they are different claims. The ledger **never resets between sub-games** (the obvious per-game counter is zeroed exactly where a crash interrupts); a repeat `record` is refused rather than summed (a retry would inflate the count); an over-run is reported, never clamped; and a provider response with no usage block is refused, because a provider that stopped reporting looks exactly like a game that used no tokens `[AE-54]` |
...
| M7-013d | Document the five setup steps for a fresh machine | DONE | `docs/RUNBOOK_reporting_setup.md` — five steps, with the credential check run **before** the token exists. States plainly that steps 1-3 are the operator's to run and that no `authorize` command exists, which is why `M7-013`/`M7-013a` stay unclaimed; gives the exact `InstalledAppFlow` snippet instead of documenting a command that is not there |
| M7-014 | Compose the report email | DONE | `compose_report` + `reporting/gmail_wire.py` (100% branch). Subject carries the 8-character team code `[AE-45]`; body carries no part of the report `[AE-34]` |
| M7-014a | Attach the result artifact as a file | DONE | `test_gmail_wire.py` asserts the attachment survives the **round trip** — parsed back out of the encoded bytes to its JSON — rather than stopping at "the bytes match" |
...
| M7-014c | Base64url-encode and send through the API | DONE | `send_body` / `api_send` with `userId="me"`; `test_gmail_send_call.py`. Two silent mistakes tested directly: **base64url vs standard base64** (the alphabets differ only in `+/` vs `-_`, so most messages encode identically and only some diverge — the probe is built from `~?` at every residue class mod 3 to force the difference, because a composed report cannot: its payload is already base64), and **kept padding** (the stripping idiom is JWT's, a different specification). Found two real test bugs: MIME boundaries are random per message, and the obvious fixture could not tell the alphabets apart |
...
| M7-018 | Run a full local series rehearsal before any counted game | DONE | `tests/integration/rehearsal.py` + `rehearsal_fixtures.py`, driving the **real** builders, audit, settlement, ledgers and retention store; only the transport is a double. `test_rehearsal.py` (9 tests): every sub-game, four artifact families, one shared `game_uid`, every file written and read back, audit-then-agreement, mocked send with `userId="me"`, every config committed and retrievable, both rule 54 figures, four repository links `[AE-49]` |
| M7-018a | Rehearse with a deliberately failing sub-game | DONE | `test_rehearsal_failures.py` (11 tests). A technical loss produces **exactly the same file set** as a clean series — asserted as `sorted(artifacts) == sorted(clean.artifacts)`, not merely "artifacts exist", because a pipeline dropping one file per bad sub-game passes the weaker claim. The loss shows in the score and is named in the result; the forfeited sub-game keeps its own log with commitments, and the report is still sent `[AE-32]` |
| M7-018b | Rehearse with a tampered audit | DONE | `test_rehearsal_tampering.py` (10 tests). A revealed move rewritten **after** its commitment — nothing looks malformed, only recomputing the digest shows it. Detection names the sub-game; the state is `audit_failed`, not `conflict`; and **nothing is sent** — racing them to the lecturer converts their rule 19 loss into a shared rule 35 loss. Artifacts are still written: refusing would destroy the evidence of the forgery |
| M7-019 | Document the reporting pipeline | DONE | `docs/PRD_gatekeeper_reporting.md` rewritten from "future acceptance criteria" to the built pipeline — flow diagram, a requirement-to-code table with the rule for each row, and a **Deliberate departures** section recording the three places the built design differs from the obvious one (table not JSON Schema, `schema_version` held at 1.1, `reporting/` importing no transport) with the reason each was chosen |
...
| M7-023 | Keep artifact emission independent of transport health | DONE | `test_transport_independence.py` (structural) + `test_disconnected_emission.py` (behavioural). Nothing under `reporting/` imports `adapters`/`peer`/`sdk` or any network library, read from the **AST** rather than by importing — an import-based check passes once an earlier test has loaded the module. The four-artifact set builds and writes with `socket` replaced by a raiser and `confirmed: False`, which is the realistic case: the game whose evidence gets disputed is the one that went wrong |
...
| M8-003c | Rehearse against a real classmate agent before the counted league | DONE | Warm-ups are permitted and uncounted `[AE-52]`. **Deliberately still open, checked 2026-08-09.** A friendly series against group `amireman` did begin that day, but sub-game 1 was played in the **Police** role, so only the companion repository has met a live opponent — this agent's code has never run against one, and the absence of any `games/friendly-amireman/` here is the evidence of that. Marking it `DONE` on the strength of a game the companion played is precisely what **`THIEF-002`** forbids — no task may be satisfied by the companion Cop repository — so the two rows disagree on purpose. Closes on the first sub-game played in the Thief role — sub-game 2 of the running series, or a series started with the Thief. **CLOSED 2026-08-12 (evening):** this agent played six Thief-role sub-games against `uoh-ay26` across two complete six-game series (`games/friendly-uohay26-0812-1934/`, `-0812-2201/`), surviving the full horizon in all six, with every audit exchanged and — in the second series — zero wire rejections end to end. Both sides read the series 90–30 in writing |
...
| M8-016 | Draw the replay board: the whole chase at a glance | DONE | **Closed 2026-08-08.** `replay/board.py` (pure view model) + `ui/replay_board.py` (drawing) + the board panel, `Play` auto-advance, and opponent-log support in `ReplayWindow`. Authority checked first: rule 9 binds the **live** interface; the replay is the "Retrospective Witness" (p.54/135), and the reference itself draws both true positions when the opponent's log is present, falling back to one trail when it is not. Trails fade with age, barriers appear as placed (parsed from our `state` string **or** a companion-shaped `payload.barriers` list), a capture rings the cell we were caught on. Screenshots regenerated **from the real rehearsal match** (`game-593df753457f`, both logs, cross-loaded). The real log also exposed a latent defect the fixture logs had hidden: `view_model`/`sequence` read `step`/`sender`/`move` only at the record's top level while our own emitted log seals them inside `payload` — every row rendered `step ? — —` and 21 numbered steps reported as "unnumbered". Both now fall back to the sealed payload, and the sender column fills from the log's declared role. `test_replay_board.py` (7). **Styling pass in the same batch:** `ui/style.py` — dark-navy chrome, glowing pill banners, rounded board cells and cards, hover-lit flat buttons — pure tkinter, no theme dependency, re-authored here rather than copied (`THIEF-002`); verdict colours and the heat ramp stay untouched (reference-matched, test-pinned). The styled window crossed the 150-line cap and split its panels into `ui/replay_panels.py`. **App icon:** `scripts/generate_icon.py` writes `assets/icon.png` (our amber disc on the dark tile, blocky `T`) with `zlib`+`struct` alone — deterministic, dependency-free — and `style.apply_icon` puts it on every window behind a guard, because chrome must never be able to stop a match |
...
| M9-005 | Create the reviewed annotated `v1.0-submission` release tag | DONE | Tag object `f2a4909` — a real annotated tag, not lightweight — points at `e0b8237` and is present on `origin`. **Re-point it if the submission commit moves**: it currently equals `origin/main`, so any further commit leaves the tag behind, and rule 41 wants the tag on the commit actually submitted `[AE-41]` |
...
| M9-007c | Cite academic references and format equations in LaTeX | DONE | Four formulas rendered in LaTeX as guidelines `:741` requires: the Dec-POMDP eight-tuple (p.4/109), the scent update (p.27/115), the Bayes belief update (p.48/123) and the token bucket (Appendix F table 19). **Discloses a contradiction in the source**: `inst/:930` says (1-rho) means scent is 'reduced by 90%' while its own formula retains 90%. We implement the formula — the rule 23 lock is over the formula, and the prose reading decays ten times too fast |
...
| M9-011 | Write the academic report body | DONE | `docs/ACADEMIC_REPORT.md`, linked from the README. The formalism in the book's own notation, every architectural decision with what it cost, and the measured results |
| M9-011a | Justify the architectural decisions and trade-offs | DONE | Five decisions, each with **what it cost**, not just what was chosen: the model never picks a move (forfeits tactical contribution, buys reproducibility that makes rule 20's replay mean anything); lexicographic ranking with gamma left unused and said so rather than inventing a discount to look complete; a citation table instead of a JSON Schema (cannot hand an opponent a schema file); refusal over accommodation at every boundary (more interruptions reach the operator); reporting imports no transport (the send path is assembled by the caller, and `test_report_precondition.py` exists because that step was got wrong) |
| M9-011b | Present empirical results, not claims | DONE | Measurements, not claims. **Reports the result that does not flatter us**: over 24 scenarios the belief policy survives 51% longer and scores 20% fewer league points (175 blind / 140 belief), because Appendix F pays for reaching the threshold or capture with nothing between. `M6-015c` left open rather than patched — patching a metric after seeing which arm it favours is how a result stops meaning anything. Decision cost 0.86 ms mean / 2.11 ms worst on 7x7, 0.012% of the negotiated timeout |
| M9-011c | Disclose every book contradiction relied on | DONE | **Corrected 2026-08-07 after being closed on an incomplete list.** `docs/HANDOVER.md` now discloses **six** contradictions under chapter 110, not four: draft-vs-send, the duplicated `[Number of Agents]` row, `schema_version` held at 1.1, which artifacts must be committed, plus `C-014` (the scent factor prose says 'reduced by 90%' where the formula retains 90%) and `C-015` (raising rho is said to saturate the board; it empties it). Both had been in `docs/SPECIFICATION_CONFLICTS.md` since M6 and were never promoted, so a row reading "every book contradiction relied on" was marked done against a four-item list. The register is the source and the handover is a view of it; the section now says to diff the two before closing this row again |
| M9-011d | Cite the reference list | DONE | Ten numbered references in the guidelines' own bibliography format (`inst/:1082`) — the book, the guidelines, ISO/IEC 25010, Oliehoek & Amato for the Dec-POMDP formalism, Naor for bit commitment, FIPS 180-4 for SHA-256, Dorigo for stigmergy, plus the guidelines' own Nielsen and API-key entries |
...
| M9-014c | Evidence integrity | DONE | Integrity: commit-reveal over canonical bytes, audit-before-agreement structural, nonces refused pre-`ended_at`, stored match re-verifies off disk, 2050 history objects clean |
...
| M9-015b | Confirm the cross-link to the companion repository resolves | DONE | `github.com/SharbelMaroun/p2p-cop-agent` appears twice in the README and is asserted by `check_submission_contents.py` (book 9.4.2 item 6). A link only; no repository access is claimed or needed `[AE-49]` |
...
| M9-019 | Verify the annotated tag points at the reviewed commit | DONE | `scripts/check_submission_tag.py` + `tests/unit/test_submission_tag.py` (5 tests). Four checks in order: the tag exists, is **annotated** rather than lightweight (a lightweight tag is a bare pointer with no author, date or message — the exact provenance rule 41 asks for), nothing was committed after it, and the tree is clean. The third is the one that earns its keep: the mistake is not forgetting to tag, it is tagging and then fixing one more thing, which leaves a tidy repository whose tag names a commit nobody reviewed. Exits 1 with the `git tag -a` command until the tag is made, which is the right answer to "have I submitted correctly" while the answer is "not yet" `[AE-41]` |
...
| M9-024 | Close out the prompt-engineering log | DONE | `docs/PROMPT_LOG.md` closeout. Four lessons, each paid for: skipping step 3 is invisible until it is expensive; a notebook answer is a lead not a source (the book notebook has fabricated at least twice); asking two notebooks *different* questions is worth more than asking one twice — the best findings came from the gap between what is required and what the reference does; and **the shortest prompt produced the most value** — "there is a commit at least that faild the github actions" found four red builds and a security gate reporting 0 findings on a shallow clone |
| M9-025 | Give the agent a command-line runtime | DONE | **Closed 2026-08-07.** `src/p2p_thief_agent/cli.py` gains `serve`, `replay` and `verify`; `tests/unit/test_cli_runtime.py` (12 tests). Verified end to end against a real rehearsal artifact: `p2p-thief replay --log …` prints `Verified OK — 3 steps re-verified`, exit 0. **The gap was wiring, not behaviour** -- every piece existed. Two properties tested because both fail silently: `build_parser` imports no transport (asserted from the AST, since every other test has FastMCP installed and would not notice a drift), and `verify` **exits non-zero** on TAMPERED -- a gate that prints the word and returns 0 is worse than no gate, because a pipeline reads the code not the prose. Exit codes are distinct: 0 verified, 1 tampered, 2 unreadable. **Still not possible: playing a full counted game from the CLI** -- `serve` binds and waits, deliberately, because this peer opens every turn cycle and would race an opponent still binding its port `[AE-20]` |
| M9-026 | Let the Thief play a match, not only listen | DONE | **Closed 2026-08-07.** `adapters/serve.serve_match` + `p2p-thief serve --peer URL`. `M9-025` gave this repo a command line that could bind a mailbox and wait; nobody ever started a game, because starting one means negotiating and there was no path from the CLI to the turn loop. Verified end to end: a real match ran against a live mailbox on another port, decided, and wrote its log. **Three layering tests decided where the policy adapter lives** -- not `strategy/` (it seals, and `test_strategy_sdk` refuses a strategy importing `protocol`), not `adapters/` (`test_orchestrator_boundary` refuses one subsystem importing another), so `orchestration/thief_policy.py`. Both tests fired before the file settled anywhere else |
| M9-026a | Make the live `decide` play the measured policy | DONE | **Closed 2026-08-08.** The adapter `M9-026` wired was the blind baseline wearing the live loop's clothes: `baseline.choose_action` with no threats and no barriers, `incoming` ignored entirely, `smell_grid` hard-coded `{}` (the rule-23 deviation the companion fixed on its own side on 2026-08-06), a constant hint, and no belief anywhere — every M6 number existed only in the harness while the wire played the arm that measures **worse than a random walk** (`M6-015c`). `make_decide` now: absorbs `barrier_placed` (trusted — disclosure is a truth duty) and the Cop's `smell_grid` into a belief rebuilt **fresh per observation** and carried only across silent turns (the Bayes-recursive form calcifies without a motion model — measured in the companion's opponent grid, 40/40 → 0/40 tracking on that change alone; fresh is also exactly what the `M6-015` harness arm measures, so the live Thief plays the policy the published numbers are about); evades via `choose_evasive_action` around the disclosed barriers; emits the involuntary 5×5 own-trail window; seals real position/move/hint; carries `win_claim` on the threshold step so the opponent terminates on our survival instead of timing out into a disputed artifact. `tests/unit/test_thief_policy_live.py` (10) |
| M6-029 | Commit the pursuer archetypes and the exact escape solver | DONE | **Closed 2026-08-08.** `strategy/pursuer_models.py` (greedy = byte-for-byte the harness Cop, herding, anticipating — the report's stronger rows existed only as table numbers from scratch code) + `strategy/escape_search.py` (memoised `(cop, thief, steps_left)` recursion under the harness order; seeded with truth it is the ceiling solver, seeded with belief it is a legal planner). Committing the models immediately corrected the record: the committed herding and anticipating are **stronger** than the 2026-08-07 scratch versions — shipped evasion measures **23/24 greedy, 8/24 herding, 5/24 anticipating** (was 23/23/8 against scratch). `test_pursuer_models.py` (6), `test_escape_search.py` (7), incl. the 5×5 ceiling row reproduced exhaustively |
| M6-030 | Sixth attempt at the anticipating gap: classify online, best-respond exactly | DONE — **negative with the raw belief; wired 2026-08-08 together with `M6-031`, which flips the verdict** | **Closed 2026-08-08.** `strategy/adaptive_policy.py` + `scripts/experiment_pursuers.py` → `results/pursuer_grid.json`. Argmax-fed: 23/4/4 vs shipped 23/8/5 — worse; top-2 uncertainty set: worse still (2/24 herding); robustness (9×9, horizon 50) agrees. Not shipped, per the `M6-015b` reversion rule. **The finding that outlives the failure: truth-fed, the same machinery escapes 24/24 against all three archetypes** — the planner and classifier are provably correct, so the report's old claim that belief is "not the bottleneck" is refuted: the ~1-cell argmax error is exactly what reduces a perfect planner below a robust heuristic, and all six failed attempts share that one diagnosis. Attempt #7 should sharpen the **estimator** (the emission physics are known and deterministic — a model-matched likelihood can localise the emitter far better than raw intensity weighting), then re-run this grid — one command, because this time the failed attempt's machinery is committed. `test_adaptive_policy.py` (7: history-determinism, truth-fed ceiling, exact shipped-fallback equality, legality, memo invalidation) |
| M6-031 | Seventh attempt: invert the scent physics — the model-matched emitter decoder | DONE — **closes the gap: 240/240 on every grid cell** | **Closed 2026-08-08.** `perception/emitter_decoder.py`. The sixth attempt's diagnostic said the estimator was the whole gap (truth-fed 24/24, argmax-fed 4/24); this is the estimator fixed. The locked physics `τ' = (1-ρ)τ + Δτ` has non-negative terms, so the clip never bites and the **residual between consecutive observations is exactly the newest emission stamp**; matching it against the agreed profile localises the emitter — the true cell scores zero error, the best rival carries at least `(0.9-0.62)²`, and the sharpened likelihood separates them by >100×. Authority checked before building: the book fixes the physics and frees the inference ("a free strategic component expected of every team", pp. 48/121, 94/211; Adaptation is a graded metric), and the reference itself ships a model-matched `BeliefGrid.observe_smell` with diffusion — so this is the book's own path, not a loophole. **Grid result** (`results/pursuer_grid.json`): decoded belief alone lifts the shipped policy 5→18/24 vs anticipating; decoded + adaptive (`M6-030`) scores **24/24 against all three archetypes — 240 league points — and 32/32 / 24/24 on the robustness configs**, equalling the truth-fed ceiling as a legal agent. Wired into the live loop with partial-window handling (score only cells both windows covered; stale observations fall back to single-stamp matching) and a deviation guard: a field the model cannot explain anywhere yields explicit no-information, never a confident wrong answer (`M6-003c`). `test_emitter_decoder.py` (9: residual exactness, whole-walk exact tracking incl. revisits, partial windows, degradation, determinism) |
| M6-032 | Wall awareness: the interceptor model, the wall-pressure guard, and the waller boundary | DONE — capability shipped; the interception-waller boundary is measured and documented, not closed | **Closed 2026-08-08, tournament hardening.** Three pieces. (1) `pursuer_models.interceptor_step` — close on the whole flight set by **summed** barrier-aware step distance (the centroid chaser cannot price a spread, so an edge-bobbing runner ties it; the sum crosses) — registered fourth in `PURSUERS`, so the classifier can fit it and the exact solver plans against it. (2) `metrics.wall_pressure` — exits kept after the believed Police's best single in-range wall (book §3.4 + `AE-046`), graded: 0 = one wall ends the game (also any target inside walling range), 1 = the seal cascade. Leads the adaptive ranking unconditionally; far from the threat it degrades to plain mobility, so the mover grid is untouched by construction. (3) `scripts/experiment_wallers.py` → `results/waller_grid.json` — the first grid in which the Police spends quota: greedy and interceptor movers wrapped with finishing/sealing walls (instruments; own-cell walls left out as a simplification). **Measured**: vs `greedy_waller` 23/24 (the reference-shaped classmate with walls changes nothing); vs `interceptor_waller` **8/24 (16/32 at 9×9) — unchanged by the pressure guard, by a first-wall regime switch, and by risk-first promotion, all three measured**. The boundary is structural: an interception pursuer collapses the escape space from beyond walling range before any in-range refusal can matter, and the companion repository measured the same fact from the other side of the board at 40/40. Recorded like the Cop's old flee_smart 0/40 was: a measured limit, stated with its mechanism, not tuned at. **The mover grid re-ran with the pressure lead in place and held everywhere** (`results/pursuer_grid.json`): `adaptive_decoded` **24/24 against all four models — interceptor included — 240 league points per cell, 32/32 and 24/24 on the robustness configs, equal to the truth-fed ceiling**, so the guard measurably costs nothing where the boundary does not bind. `test_wall_awareness.py` (9) |
| M6-033 | Live fail-safe: a strategy exception costs one turn, never the match | DONE | **Closed 2026-08-08.** An uncaught raise in the live `decide` reaches the watchdog as a freeze and scores the technical 0/0 — strictly worse than any legal move (a captured Thief still pays 5). The evade step now converts any strategy exception into a truthful sealed `MOVE:STAY` from the held cell; the trail still advances and every mandatory wire field still goes out, so the game and its audit continue. `test_wall_awareness.py::test_a_raising_strategy_yields_a_truthful_stay` |
| M6-034 | Exact walled escape solver + walling archetypes in `src` | DONE | **Closed 2026-08-14.** `strategy/waller_models.py` lifts the deterministic walling pursuers into `src` (byte-parity with the committed `experiment_wallers` grid — 8064 states identical — and every wall they propose passes `domain.barriers.validate_barrier_placement`), and `strategy/barrier_search.py` is an exact escape solver that carries the barrier mask and quota **inside** the recursion, mirroring `escape_search`'s step order. Cross-checked: with quota 0 the walled solver equals `escape_search.survives` exactly. Inert — no live caller — so gameplay is unchanged. **Measurement that decided the next step: the solver fed the true Cop cell escapes all 24 openings against both wallers (24/24 ceiling), refuting `M6-032`'s note that the interception waller is structurally winning from these openings.** `test_waller_models.py` (9), `test_barrier_search.py` (9) |
| M6-035 | Barrier-aware evasion v2: always-on walled planner, default-off | DONE — experimental, SHIP_CANDIDATE; production default unchanged | **Closed 2026-08-14.** `strategy/barrier_aware_policy.py` keeps the shipped adaptive pick whenever it already survives the assumed waller, substitutes a walled-safe action only when the adaptive pick would be trapped, and falls back to the adaptive pick otherwise — so the shipped policy is both default and floor. **A first cut gated the planner on danger and recovered nothing (8/24 unchanged, zero overrides): against a walling interceptor the escape space is lost before a wall is placed, so the gate opened too late.** Planning every step converts `interceptor_waller` **8/24 → 24/24** and `greedy_waller` **23/24 → 24/24** on the decoded belief, at every depth from 6 up, while all four mover archetypes hold **24/24 (no regression)**; worst decision (depth 8) stays far inside the response budget. Wired behind a private `[strategy].policy` selector — `make_decide(strategy=...)` dispatched by `evasion_action`; `"current"` (the default, and any unknown value) is `choose_adaptive_action` byte for byte. **Production default remains `"current"`; flipping it is the coordinator's decision.** Deterministic (book §6 sanctions deterministic minimax/expectimax), replay- and audit-safe. `scripts/experiment_barrier_aware.py` → `results/barrier_aware_grid.json`; `test_barrier_aware_policy.py` (10), `test_evasion_selector.py` (4) |
| M9-027 | First two-process rehearsal of the real policies — and the artifact that stopped lying | DONE | **Closed 2026-08-08.** Cop and Thief as two OS processes over localhost HTTP, `serve` CLIs on both sides, the shared byte-identical match JSON, real identities, real strategies. Four findings, each fixed and re-rehearsed: the playable path skipped negotiation (`M5-020`); the serve receive never waited (`M5-020`); the companion replied to a decided game (its `M5-18`); and this repository's local-match log hard-coded `"winner_role": "thief"` — a false claim in a signed artifact whenever we are captured — now derived from the outcome (`adapters/match_log.py`). Final run: **negotiation agreed, 35 commit-reveal turns, both sides record SURVIVAL after 35, and the log replays `Verified OK — 35 steps re-verified`** with real evasive movement and real generated hints on the wire. **Scope stated honestly:** this is the engineering rehearsal; the book's league checklist requires "an accessible address, not only localhost" (p. 97/215) plus Live-GUI/Replay screenshots (p. 81/189), so the tunnel rehearsal (`M5-005c`, `M8-003c`) remains the operator's step |
| M9-029 | Play a battery of real wire matches and verify every artifact | DONE | **Closed 2026-08-08.** Eleven two-process matches over live HTTP: the six-sub-game series, adjacent/close starts, 9×9, a **negotiated 50-step horizon** (both peers played exactly 50 — the terms projection, not a local default, governs), and a corner press. **11/11 outcome agreement; 22/22 logs replay `Verified OK`.** Ten Thief survivals (incl. from one cell away — the Thief opens and never returns the head start) and one **capture at turn 21** from the cornered start, exercising claim → honest confirmation → mutual termination → a *police*-winner artifact over the wire. Fresh mandatory captures: replay `Verified OK` over the capture game's real log; live-GUI belief map from a real socket exchange. Ops lessons recorded in `PROMPT_LOG.md` (orphaned monitor processes held log files open; a forgotten detached driver raced the games — every affected game re-run cleanly) |
| M9-028 | Counted-game evidence from the wire path, and the match runbook | DONE | **Closed 2026-08-08.** Two gaps between "a match runs" and "a match counts". (1) **Truthful artifacts:** the wire log carried placeholders (`local-match`, opponent `"opponent"`, unconfirmed zero sha) even though negotiation held the real values. `negotiated_agreement` now returns the whole `AgreedMatch`; `serve_match` derives the cross-peer game identity from the shared file's canonical sha — **verified: both repositories derived `game-9934e8338307` independently**, so the two canonical-JSON implementations agree — and threads opponent id, config lock, `confirmed: true`, true start time and `--sub-game` into the log. (2) **`docs/MATCH_RUNBOOK.md`:** the one-page classmate procedure — byte-identical shared-file handshake with the hash command, both sides' exact serve commands, the ruled 1/3/5-natural 2/4/6-swapped schedule as six fresh runs, rule-51/commit/screenshot duties after, and the troubleshooting list every entry of which cost one real rehearsal run (`agreed_between` naming, UTF-8 BOM, unreachable peer, differing files). Validated end to end: full match, both logs replay `Verified OK` — including the Cop's log **cross-verified by this repository's own replay tool** (34 steps re-verified), two independently written implementations checking each other's cryptography |
| M9-026b | Answer capture claims honestly, from one closure | DONE | **Closed 2026-08-08.** Two defects, both audit-fatal under `[AE-021]` (a false denial is proven from our own sealed positions and scores a forgery — zero for both sides, where an honest loss still pays 5): `serve_match` defaulted `answer_claim` to `lambda _cell: False` — a standing denial of every correct capture — and the CLI never overrode it; and the sub-game loop asks `answer_claim` **after** the turn's move is applied, so even an honest live-state comparison would test the claim against the cell we fled to. Now: no default — an absent answerer is a refusal to play (`ServeError`), `make_decide` attaches its own answerer sharing the position closure, claims are answered from the **pre-move** cell, a confirmed capture pins the move to `STAY` so the sealed record shows the cell we were caught on, and the wire message carries `claim_response` the same turn, closing the loop that would otherwise leave the Cop timing out (0/0) on a game we recorded as a clean capture |
| M9-030 | Close the external pre-submission audit — stale documents against current code | DONE | **Closed 2026-08-08.** An independent examiner reproduced every gate (1591 tests, 95.58% branch, ruff clean, lengths, secrets over tree + 2050 history objects; the contract checker correctly fail-closed at `PENDING`) and found **no disqualification-level violation** — the commit-reveal and scent-lock digests reproduce byte-identically against the companion. Two real defects: (1) `results/strategy_comparison.json` claimed belief survival **125** while re-running `run_comparison()` returns **140** — the `M6-032` wall-pressure term improved the policy and the committed result was never regenerated, so the README quoted a number this repository could no longer produce; every other result file reproduced exactly. (2) **README §4 argued against its own data**: it still told the metric-disagreement story with belief losing the league 140 to 175 and `M6-015c` open, though the research report and academic report were corrected on 2026-08-07 and `results/strategy_arms.json` carries `metric_disagreement: false` with belief winning **235 to 175**, 23/24 escapes, paired 13–0–11. Three documents, two stories, and the flag built to catch it already read `false`. Also refreshed: test/coverage/history counts, and the machine-dependence of the decision-cost figures stated rather than implied |
| M9-031 | Stop the charts from asserting conclusions nothing re-checks | DONE | **Closed 2026-08-08.** `chart-metric-disagreement.svg` was titled "The two metrics rank the strategies in opposite directions" while drawing 235 against 175 in the **same** direction — the title and caption were hard-coded strings in `render_charts.py`, a conclusion about a measured quantity with an expiry date nobody was watching. Both are now computed from the same numbers the bars are drawn from (`_metric_title`, `_metric_caption`), so the picture cannot contradict itself again; the alt text in both documents follows the chart. Same class of defect as the stale README table, found by the same question: *what re-checks this sentence?* |
| M9-032 | Make the mandatory screenshots evidence a grader can reproduce | DONE | **Closed 2026-08-08.** The committed `Verified OK` capture showed a log living in a **temporary directory**, captioned as if it showed a test fixture. Asked directly, the book requires these captures to show a game **actually played**, not a fixture. The played match `game-593df753457f` is now committed under `games/` — configuration (Appendix F obligation 4), declaration, this peer's revealed log and the companion's (rule 36 mutual audit, and the viewer's two-trail board) — all verifying under both peers' verifiers. The capture script reads the committed log and lets Tk size the window, which fixed a crop that had been hiding the transport controls and the last rows of a 21-step match |
| M9-033 | Restate `THIEF-002` as the wire-input discipline it actually is | DONE | **Closed 2026-08-08.** `THIEF-002` was written as "developed with no read and no write access to the companion Cop repository". The discipline is real — the wire is matched against the book and the pinned simulator, which is why the protocol and strategy layers genuinely diverge — but both repositories are written by the same team and share ~33 support files, one byte-identical **including its dated discovery note**. No rule forbids that: rule 49 asks one group for two repositories, and rules 1–2 govern **run-time** separation, which is structural and tested. `docs/SHARED_MATERIAL_AND_AUTHORSHIP.md` itemises what is shared and why a shared package would be the wrong fix; the ledger entry, README and contributing note now say wire *inputs* rather than authorship |
| M9-034 | Add the guidelines section 11 cost and resource analysis | DONE | **Closed 2026-08-08.** The academic report emitted rule 54's token figures but had **no cost section at all**, which guidelines section 11 asks for. Added as section 3.5: the token table (the shipped template provider is **0 tokens**, against Appendix F's ~200 000 estimate), and the case that this is a competitive position rather than a corner cut — the book grades *computational fairness*, asking whether a phone races a workstation fairly, so 24/24 against every committed archetype at zero tokens and under 4 ms per decision is evidence for exactly that. What it costs is stated too: no rhetorical sophistication in the hints and no claim to an LLM-driven strategy |
| M9-035 | Make the submission-tag test able to fail | DONE | **Closed 2026-08-08.** The old tests asserted `main() in (0, 1)` — true of any function returning an int — and `isinstance(tag_exists(), bool)`, and one **returned early the moment a tag existed**, so the suite went quiet at exactly the point the 'tag names an older commit' failure becomes possible. A test that switches itself off when the risk appears is worse than no test. Rewritten to build **real throwaway Git repositories** and drive the checker at each branch: untagged, correctly tagged (the case the old suite could never reach), lightweight tag, a commit made after the tag, and a dirty tree. Plus one unconditional check that *this* repository is tagged and annotated |
| M9-036 | Close the `ast.Import` hole in the rule-8/9 boundary guards | DONE | **Closed 2026-08-08.** Same defect as the companion's, found by the same audit: the truth-boundary walkers matched only `ast.ImportFrom`, so a plain `import p2p_thief_agent.orchestration as o` inside `live/` would have passed the one test whose sanction is disqualification of the project. `tests/unit/boundary_imports.py` now walks both statement forms |
| M9-037 | Match `target-version` to `requires-python`, and pay what it surfaces | DONE |**Closed 2026-08-08.** ruff was pinned to `py310` against `requires-python >=3.11`, judging the code against a Python this project does not support. Correcting it surfaced 8 real findings: two import blocks, two `datetime.UTC` modernisations, and four `UP042` enums converted to `StrEnum` with the full suite as arbiter (JSON output is identical; only `str()` differs). Consolidating a `datetime` import kept `adapters/serve.py` inside the 150-line cap rather than widening the gate |
| M9-038 | Report what is armed, instead of adding a switch that claims it | DONE |**Closed 2026-08-09.** `services/preflight.py` + `p2p-thief preflight --match --private`: one screen carrying version, private config, both endpoints, **port free**, match config with its Appendix F verdict, the scent-lock digest, and reporting `ARMED`/`DISABLED` with the credential path it checked. Exit 1 on any failure so a script can gate on it. **Deliberately not a toggle**: sending is already impossible three structural ways (no credential, no CLI verb reaching the sender, no call in the play path) and a boolean is weaker than any of them — it can be defaulted wrong or read from the wrong file. What was missing was visibility: proving reporting was off took four greps across two packages. The port check is the reference's `_ensure_port_free` (`infra/mcp_server.py`), which exists because a stale peer holding the MCP port fails as a bare `WinError 10048` — our own rehearsals lost runs to it and the symptom was the *opponent* looking absent. Seven tests, every check driven to **both** verdicts |
| M9-039 | Delete the `[email]` block, a control wired to nothing | DONE |**Closed 2026-08-09.** Both templates carried `[email] mode = "draft"` and **no code has ever read it**; `recipient` is read nowhere either, appearing only in the forbidden-keys guard and as a `SendReceipt` parameter. A switch wired to nothing is worse than no switch — it invites someone to believe reporting is off because a file says `draft`, when what actually stops a send is the absence of a credential. Removed from both templates with the reason recorded in place, and pinned by a test that **parses the TOML** rather than matching text: its first version failed on the comment explaining the removal |
| M9-040 | Make readiness mean *served*, not merely *routable* | DONE |**Closed 2026-08-09, found by a live match attempt.** `serve_and_play` waits up to `connect_timeout_seconds` for a late opponent, and **the wait never ran**: the probe was `port_answers` — a TCP connect to the host/port parsed from the opponent's URL — and through a tunnel that host is a **CDN edge that accepts on 443 whether or not the peer exists**. Proved live: against an endpoint returning 502, `port_answers` returned `True`. So the probe said ready, the wait was skipped, and the first `negotiate` died on the 502 the wait existed to absorb. Replaced with `peer_answers(url)`: 502/503/504 mean no origin, anything else — including the `406` an MCP endpoint returns to a bare GET — means present, **preserving the old docstring's point** that "not up yet" stays separable from "up and refused". Correct on localhost, wrong behind a CDN, and every rehearsal was on localhost. This peer carried the same defect with an extra edge — it **defaulted to port 80** when an https URL named no port, probing the wrong port of the right CDN. 10 tests, including one asserting the two probes disagree exactly where the bug was; a malformed-URL case immediately caught `Request()` raising outside the `try` |
| M9-041 | Cap every outbound call below the signed deadline | DONE | **Closed 2026-08-10, the same defect the companion Cop carried.** `FastMCPClient` accepts a `timeout`, but `serve` built it as `FastMCPClient(peer_url)` with none, so `self._timeout is None` and every live call waited indefinitely. The breach is arithmetic rather than networking: the MCP SDK's own per-call default equals the 30s we sign as `response_timeout_sec`, so one delivered-but-unanswered push, one backoff and a second push exceed the deadline **while each individual call looks healthy** -- we breach a deadline we signed and score ourselves the technical loss. `RetryPolicy.call_timeout_sec` derives the cap as the signed budget over `attempts`, the largest value for which the whole retry sequence still fits. Two things the tests caught rather than the design: at **zero** negotiated retries `attempts` is 1 and the division hands one call the entire deadline, so a `MARGIN` clamp keeps it strictly under for every retry count; and the limits sit in **two different sections** -- the deadline at `network_and_league.response_timeout_sec`, the retry count at `rate_limiter_gatekeeper.max_retries` -- so a config writing both under one section silently falls back to Appendix F's default instead of failing. `_call_timeout` also bounds the **unnegotiated** path, which reaches the transport before any config is agreed. 10 tests in `test_call_timeout.py`. This repo's turn `timestamp` needed no change: `now_iso()` has emitted ISO-8601 since it was written, matching the reference's `peer/sealing.py` |
| M6-005d | Correct the scent emission kernel against the book PDF | DONE | **Closed 2026-08-08.** The 5x5 emission table was the correct radial curve **shifted inward by one class**: the diagonals are `0.42` not `0.20`, the mid-side `0.20` not `0.14`, and the eight squared-distance-5 cells `0.14` rather than a negotiated residual -- Figure 4 names all 25. Raised by a classmate team, confirmed by the book PDF, by a zero-parameter fit of `tau = 0.9*exp(-k*d^2)` through the two agreed values (`0.427/0.203/0.140/0.046`), and by their independent kernel. **`U-025` had closed against these exact values on 2026-08-05** on the grounds that a notebook answer is not a source -- but the notebook holds the PDF and the file it was checked against is a translation. Lock digest `416a57e1...` -> `e6aef097...`, still identical to the companion's, so the cross-peer tripwire held throughout. Kernel, regression matrix, PRD, ledger and every measured result regenerated `[AE-23]` |
| M6-005e | Answer the interop questions the classmate analysis raised | DONE | **Closed 2026-08-08.** Three claims checked and not adopted: `game_id`/`game_uid` **are** derived deterministically here (they read the dataclass, not the call site), though our derivation differs from theirs and the book mandates neither -- a per-match negotiation item; their report consensus signature (spaced separators, Hebrew key) is in no book section and its spaced separators contradict the canonical-JSON rule the book does state; and their open question is answered by the code -- `smell_grid` rides in the **public** turn fields, never in the sealed payload, so a scent mismatch can only ever be a pre-game refusal, never a both-zero audit |
...
| `THIEF-002` | This repository does not take the companion Cop repository as a **wire** input; the pinned simulator is the sanctioned reference instead. **Restated 2026-08-08**: it governs design inputs, not authorship -- both repositories are written by the same team and share support code, itemised in `docs/SHARED_MATERIAL_AND_AUTHORSHIP.md`. The separation the rules require (rules 1 and 2) is at run time |
