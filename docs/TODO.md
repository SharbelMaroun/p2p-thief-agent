...
| M5-015 | Exchange and verify the scent-model lock at negotiation | DONE | Closed by `M6-005` once the scent model existed: `perception/scent_lock.with_scent_lock` stamps the model hash into the signed terms and `accept_offer` refuses a mismatch by name before the first move; `test_scent_lock.py::test_the_negotiation_refuses_a_scent_mismatch_before_play` `[AE-23]` |
...
| M6-005 | Lock and exchange the scent-model hash before the first move | DONE | `perception/scent_lock.py`: the full model (formula, FIXED constants, field size, and the Figure-4 emission profile incl. the `U-025` provisional) is canonicalised and SHA-256 locked, stamped into the agreed terms by `with_scent_lock`, so the agreement gate compares it and refuses a mismatch by name before the first move. `test_scent_lock.py` (100% branch). All sub-tasks below; also closes `M5-015` |
...
| M6-005b | Exchange and compare the lock at negotiation | DONE | `with_scent_lock` rides the hash in the signed terms; `accept_offer` compares it and a differing model is refused **by name** — proven end to end through `negotiate_match` (`test_the_negotiation_refuses_a_scent_mismatch_before_play`) `[AE-23]` |
...
| M7-002 | Build accepted declaration, config, log, and result artifacts | IN PROGRESS | The book-confirmed **naming and identity** are built (`reporting/naming.py`, 100% branch, `test_artifact_naming.py`): filenames derive from `game_id` (`AF-021`) and the four artifacts share one `MatchIdentity` (`AR-001`). The exact field **schemas** (`M7-002a`–`d`, `f`, `g`) are `U-019` — the observed template in `JSON_ARTIFACT_SCHEMAS.md` is unauthenticated, so they are **not built as a guessed schema** and await the coordinator's ruling |
...
| M7-002e | Share one `game_uid` across all four artifacts | DONE (naming half) | `reporting/naming.py`: `MatchIdentity(game_id, game_uid)` is the one identity every artifact carries (`AR-001`), and `match_filenames` derives all four filenames from that single `game_id` (`AF-021` / `AF-§3`). Stamping the shared `game_uid` **inside** each artifact lands with the schema builders (`U-019`) |
