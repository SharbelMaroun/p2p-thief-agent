...
| M1-014 | Define canonicalization with reproducible vectors including escaping and separated hash domains | IN PROGRESS | Vectors covering nested objects, numbers, non-ASCII, quotes, backslashes, control characters, and non-BMP codepoints |
| M1-014a | Cover nested objects, arrays, and numeric forms | IN PROGRESS | Ordering and number formatting are pinned |
| M1-014b | Cover quotes, backslashes, and control characters | IN PROGRESS | Escape handling is byte-exact |
| M1-014c | Cover non-ASCII and non-BMP codepoints | IN PROGRESS | `ensure_ascii=False` behaviour asserted |
| M1-014d | Prove the hash domains cannot collide | IN PROGRESS | Commitment and config hashes use distinct inputs |
...
| M1-015a | Assert exact tool and argument names against the stub | IN PROGRESS | A renamed tool or argument fails the suite |
...
| M1-017 | Prove fail-closed negative vectors before gameplay | IN PROGRESS | **Reopened 2026-08-06 — closed prematurely.** The parent was marked DONE on 2026-08-06 while `a`, `b` and `c` were still PENDING, and the suite that closed it does not cover them: `M1-017b` (duplicate JSON keys) is not guarded anywhere, and `M1-017c` (a private field in the **shared config**, as opposed to a turn message) has no check either. All seven categories reject, each tied to a numbered rule verified in `inst/` rather than taken from a summary — participant/config → rule 11 (Mandatory, disqualification for lack of symmetry), hash → rule 19 (Mandatory, iron rule, score 0), private leakage → rule 2 (Prohibited, immediate disqualification), replay → rule 29 (Mandatory). **Two have no rule and the file says so**: a version refuses through rule 11 because it is a signed term, and *ordering has no rule at all* — the reference does not gate ingestion on step sequence, it queues a duplicate, so the stub accepts one by default and refuses only under `strict_ordering`, which is ours |
| M1-017a | Reject altered fixed and below-minimum values | IN PROGRESS | `[AE-11]` `[AE-12]` |
| M1-017b | Reject duplicate JSON keys and unsupported versions | IN PROGRESS | Duplicates change canonical bytes `[ADR-0003]` |
| M1-017c | Reject any private field appearing in shared config | IN PROGRESS | One leakage vector per private field class |
...
| M1-025 | Keep the JSON artifact schemas documented | IN PROGRESS | `JSON_ARTIFACT_SCHEMAS.md` matches the emitted artifacts; `U-019` still open |
