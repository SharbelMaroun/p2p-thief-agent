...
| M5-007c | Bound the loop by the negotiated step limit | PENDING | `max_moves` from the match config `[AF-t15]` |
| M5-007d | Emit a structured log line per phase transition | PENDING | The log manager receives every transition |
...
| M5-014 | Implement negotiation and mismatch refusal | PENDING | Unknown-opponent acceptance works in both directions |
| M5-014a | Build and send a match offer | PENDING | Offer carries participants, terms, and the public challenge |
| M5-014b | Compare `config_sha256` byte-for-byte before play | PENDING | Any mismatch refuses the match `[AE-11]` |
| M5-014c | Validate participant identity and ordering | PENDING | Canonical and stable regardless of who proposes |
| M5-014d | Refuse below-minimum and altered fixed values | PENDING | `[AE-12]` `[AF-§1]` |
| M5-014e | Prove propose and accept directions both pass | PENDING | Neither direction requires editing a profile file |
