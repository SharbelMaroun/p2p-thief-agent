...
| M5-011 | Prove the runtime under adversarial peer behaviour | PENDING | A hostile or broken opponent cannot hang or corrupt this peer. **Checked 2026-08-02 and confirmed genuinely open** — no consolidated adversarial test file exists. The individual guards do (silence, replay, malformed, mid-audit disconnect are each covered in their own milestone's tests), so this is a *consolidation* task, not new runtime code: the companion repo's twin `M5-15` closed exactly that way, adding only the two properties the piecemeal tests never asserted — **cannot corrupt** (a rejected message leaves prior audit-bearing state intact) and **cannot hang** (sustained silence trips the watchdog into a terminal shutdown) |
...
| M5-018 | Keep transport concerns out of the SDK | PENDING | A guard test proves the SDK imports no transport module. **Checked 2026-08-02 and confirmed genuinely open** — no such test was found. Note this is *not* the same guard as `M5-002b` (which proves the transport-neutral core does not import `fastmcp`); this row is specifically about the **SDK** boundary that guidelines §4.1 makes the single entry point |
...
| M5-019e | Launch a peer as a long-running process (`serve` CLI) | PENDING | **Not done, and deliberately not stubbed.** `python -m p2p_thief_agent` is still the scaffold. Two parts remain: (1) running `build_server(...).run(transport="http", …)` — a *blocking* call — in a background thread alongside the poll loop; (2) sequencing the negotiation handshake over the wire autonomously, including the case where the opponent's tunnel is not up yet. A **passive** `serve` that only mailboxes was rejected in the companion repo on 2026-08-01 as proving connectivity rather than a game, and that decision is honoured here. Now unblocked by `M5-019`: the loop such a CLI would drive exists and is proven. **Thief-specific:** this peer *opens* every cycle, so its `serve` must send step 1 without waiting — a Thief that waits deadlocks against a Cop correctly waiting for it. **Bind address, confirmed in the book 2026-08-02 (`police_thief_p2p_Summary.md:657`): `mcp.run(transport="http", host="0.0.0.0", port=…)`, with the book's own comment "Bind the server so a tunnel can expose it publicly".** This is a trap, not a detail: `tests/integration/localhost_peer.py` binds `127.0.0.1`, and a `serve` copying that harness would start cleanly, pass every local check, and be **unreachable through the tunnel** — failing only at the stage-5 rehearsal, where it looks like a network problem rather than a one-word bug |
