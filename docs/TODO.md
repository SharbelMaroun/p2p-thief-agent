...
| M6-003 | Maintain a Thief-local belief without objective Cop truth | IN PROGRESS | Foundation built in `perception/belief.py` (100% branch, `test_belief.py`): the board-sized distribution, zero-safe normalisation, the Bayes mechanism, and the privacy guarantee. The per-hint **trust factor** and hint decoding (`M6-003b` trust, `M6-003e`, `M6-003f`) remain |
...
| M6-003b | Apply Bayes with a per-hint trust factor | IN PROGRESS | The Bayes **mechanism** is `apply_evidence` (posterior ∝ prior × likelihood, renormalised; uniform likelihood is a no-op). The **trust factor** — weighting a hint's likelihood by the sender's running trust, and lowering it when scent contradicts the hint — is `M6-003f` and builds on this |
